...
public class PLView extends Activity implements PLIView, SensorEventListener, OnDoubleTapListener
{
            ...
            (
            	new PLLoaderListener()
            	{
            		...
            		public void didBegin(PLILoader loader)
            		{
            			...
            			if(listener != null)
            				listener.onDidBeginLoader(PLView.this, loader);
            		}
					...
					public void didComplete(PLILoader loader)
					{
						...
						PLViewListener listener = getListener();
            			if(listener != null)
            				listener.onDidCompleteLoader(PLView.this, loader);
					}
					...
					public void didStop(PLILoader loader)
					{
						...
						PLViewListener listener = getListener();
            			if(listener != null)
            				listener.onDidStopLoader(PLView.this, loader);
					}
					...
					public void didError(PLILoader loader, String error)
					{
						...
						PLViewListener listener = getListener();
            			if(listener != null)
            				listener.onDidErrorLoader(PLView.this, loader, error);
					}
            	}
            );
	    	if(showProgressBar)
	    	{
	    		...
	    		(
	    			new Runnable()
	    			{
						...
						public void run()
						{
							loader.load(PLView.this, transition, initialPitch, initialYaw);
						}
	    			},
	    			300
	    		);
	...
	protected class PLInternalCameraListener implements PLCameraListener, PLIReleaseView
	{
		...
		public PLInternalCameraListener(PLView view)
		{
			super();
			mView = view;
		}
				...
				if(mIsRendererCreated && mRenderer.isRunning() && !mIsValidForTransition)
				{
					if(mHasFirstGyroscopePitch)
					{
						if(mGyroscopeLastTime != 0)
						{
				            ...
				            //ggiunta io:
				            UIDeviceOrientation newOrientation2 = mCurrentDeviceOrientation;
							switch(this.getResources().getConfiguration().orientation)
							{
								case Configuration.ORIENTATION_PORTRAIT:
									switch(this.getWindowManager().getDefaultDisplay().getOrientation())
					            	{
						            	case Surface.ROTATION_0:
						            	case Surface.ROTATION_90:
						            		newOrientation2 = UIDeviceOrientation.UIDeviceOrientationPortrait;
						            		break;
						            	case Surface.ROTATION_180:
						            	case Surface.ROTATION_270:
						            		newOrientation2 = UIDeviceOrientation.UIDeviceOrientationPortraitUpsideDown;
						            		break;
					            	}
									break;
								case Configuration.ORIENTATION_LANDSCAPE:
									switch(this.getWindowManager().getDefaultDisplay().getOrientation())
					            	{
						            	case Surface.ROTATION_0:
						            	case Surface.ROTATION_90:
						            		newOrientation2 = UIDeviceOrientation.UIDeviceOrientationLandscapeLeft;
						            		break;
						            	case Surface.ROTATION_180:
						            	case Surface.ROTATION_270:
						            		newOrientation2 = UIDeviceOrientation.UIDeviceOrientationLandscapeRight;
						            		break;
					            	}
									break;
							}
				            switch(newOrientation2)
				            {
				            	...
				            	case UIDeviceOrientationPortrait:
				            		...
				            		break;
				            	case UIDeviceOrientationLandscapeLeft:
				        	    	//Toast.makeText(this, "a2", Toast.LENGTH_LONG).show();
				            		this.doGyroUpdate(-mGyroscopeRotationY * PLConstants.kToDegrees, -mGyroscopeRotationX * PLConstants.kToDegrees);
				            		break;
				            	case UIDeviceOrientationLandscapeRight:
				        	    	//Toast.makeText(this, "a3", Toast.LENGTH_LONG).show();
				            		this.doGyroUpdate(mGyroscopeRotationY * PLConstants.kToDegrees, mGyroscopeRotationX * PLConstants.kToDegrees);
				            		break;
				            	case UIDeviceOrientationPortraitUpsideDown:
				        	    	//Toast.makeText(this, "a4", Toast.LENGTH_LONG).show();
				            		this.doGyroUpdate(-mGyroscopeRotationX * PLConstants.kToDegrees, mGyroscopeRotationY * PLConstants.kToDegrees);
				            		break;
				            	default:
				            		break;
				            }
				            if(mCurrentDeviceOrientation != newOrientation2)
				            {
				            	if(mIsValidForSensorialRotation && mSensorialRotationType == PLSensorialRotationType.PLSensorialRotationTypeGyroscope)
				            		this.updateGyroscopeRotationByOrientation(mCurrentDeviceOrientation, newOrientation2);
				            	mCurrentDeviceOrientation = newOrientation2;
				            }
						}
					}
					else
	...
	public int getDeviceDefaultOrientation() {
	    ...
	    if(rotation == Configuration.ORIENTATION_UNDEFINED) {
            Toast.makeText(this, "default_orientation: undefined", Toast.LENGTH_LONG).show();
	    ...
	    if ( ((rotation == Surface.ROTATION_0 || rotation == Surface.ROTATION_180) &&
	    	...
	    	Toast.makeText(this, "rotation:"+rotation+" config:"+config.orientation, Toast.LENGTH_LONG).show();
